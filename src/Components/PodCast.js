...
//       <div className="fixed bottom-0 left-0 right-0 bg-gradient-to-r from-gray-900 to-black border-t border-gray-700">
...
//               value={duration ? (currentTime / duration) * 100 : 0}
//               onChange={handleSeek}
//               className="flex-1 mx-4 h-1 rounded-full appearance-none bg-gray-600 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-white"
//               style={{
//                 background: `linear-gradient(to right, #FF2C00 ${
//                   (currentTime / duration) * 100
...
//               />
//               <div>
//                 <p className="text-white font-medium text-sm">{chapterName}</p>
//                 <p className="text-gray-400 text-xs">{t("podcast_upper")}</p>
//               </div>
//             </div>
...
//               >
//                 {isPlaying ? (
//                   <CiPause1 className="w-6 h-6 text-black" />
//                 ) : (
...
export const PodCast = () => {
  ...
  const handlePrev = async () => {
    try {
      const chapter = prevChapter;
      console.log("Previous Chapter:", chapter); // Debug log

      // Fix for chapter name in navigation
      const chapterNameText =
        (lang !== "en" && chapter?.[`section_name_${lang}`]?.trim()) ||
        chapter?.section_name?.trim() ||
        chapter?.chapter_name?.trim() ||
  ...
  const handleNext = async () => {
    try {
      const chapter = nextChapter;
      console.log("Next Chapter:", chapter); // Debug log
  ...
  const openSection = async (section, i) => {
    ...
    // Fix for section name when clicking
    const label =
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-center justify-between text-xs text-gray-400 mb-3">
            ...
            <input
              ...
              value={duration ? (currentTime / duration) * 100 : 0}
              onChange={handleSeek}
              className="flex-1 mx-4 h-2 rounded-full appearance-none bg-gray-600 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-white cursor-pointer"
              style={{
                background: `linear-gradient(to right, #FF2C00 ${
                  (currentTime / duration) * 100
                }%, #404040 ${(currentTime / duration) * 100}%)`,
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4 flex-1">
              ...
              <div>
                ...
                <p className="text-gray-400 text-sm">{t("podcast_upper")}</p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-6 flex-1 justify-center">
              <button
                onClick={handlePrev}
                className="text-gray-400 hover:text-white transition-colors p-2 hover:scale-110 transition-transform"
                title="Previous Chapter"
              >
                <svg
                  className="w-7 h-7"
              ...
              <button
                onClick={togglePlayPause}
                className="bg-white rounded-full p-4 hover:scale-105 transition-transform shadow-lg hover:shadow-xl"
              >
                {isPlaying ? (
                  <CiPause1 className="w-8 h-8 text-black" />
                ) : (
                  <CiPlay1 className="w-8 h-8 text-black ml-1" />
                )}
              </button>
